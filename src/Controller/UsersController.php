...
use Endroid\QrCodeBundle\Tests;
...
class UsersController extends AbstractController
{
    ...
    public function modifierProfilUser($id, UserRepository $r,UsersRepository $rr, ManagerRegistry $doctrine, Request $request): Response
    {
        // boutton te5o el id taa el user eli nzelt alih bich tmodifih
        $user = $r->find($id);
...
// mot de passe :poyoxunafomtcmpk
    ...
    //     ]);
    // }
    #[Route('/pdf/download', name: 'pdfuser')]
    public function usersDataDownload()
    {
        ...
        $dompdf->setHttpContext($context);
         $image="asset1/img/jobforyou.png";
        // On génère le html
        $html = $this->renderView('users/pdf.html.twig' ,[
                     'user' => $Userfind, 'title' => "PDF USERS DATA" ,'image'=>$image
                 ]);
        ...
        $fichier = 'user-data-'. 'Hamzapdf'.'.pdf';
    ...
    public function qrcodeUser(Request $request)
    {
        ...
        $qrcode = new QrCode('hamza');
       
         if ($form->isSubmitted()) {
            return $this->render('users/qrcodeShow.html.twig', [
               'textQR'=>$qrcode
            ]);
         }
         else{
            return $this->render('users/qrcode.html.twig', [
                'qr' => $form->createView()
            ]);
         }
