...
public class ModifierReport extends Form {
    ...
    public ModifierReport(Form previous) {
        super("Modifier", new BoxLayout(BoxLayout.Y_AXIS));
        this.previous = previous;

        currentReport = AfficherToutReport.currentReport;
    ...
    private void addGUIs() {
        ...
        typeTF.setText(currentReport.getType());
        descriptionTF.setText(currentReport.getDescription());
        
        userPC.getPicker().setSelectedString(currentReport.getUser().getEmail());
        selectedUser = currentReport.getUser();
        postePC.getPicker().setSelectedString(currentReport.getPoste().getTitre());
        selectedPoste = currentReport.getPoste();
    ...
    private void addActions() {
        ...
        manageButton.addActionListener(action -> {
            if (controleDeSaisie()) {
                int responseCode = ReportService.getInstance().edit(
                        new Report(
                                currentReport.getId(),
                ...
                if (responseCode == 200) {
                     Dialog.show("Succés", "Report modifié avec succes", new Command("Ok"));
                    showBackAndRefresh();
                } else {
                    Dialog.show("Erreur", "Erreur de modification de report. Code d'erreur : " + responseCode, new Command("Ok"));
                }
            }
        });
    }
